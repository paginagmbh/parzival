...
function attr(event, name) {
    return ["attributes", name, "value"].reduce(
...
export const filters = {
    ...
    every(...preds) {
        return (event => preds.every(pred => pred(event)));
    },

    some(...preds) {
        return (event => preds.some(pred => pred(event)));
    },
    ...
    attr(name, value) {
        return element(event => attr(event, name) === value);
    },
...
export const whitespace = {
    ...
    xmlSpace(event) {
        if (event.event == "start") {
            const xmlSpace = attr(event, "xml:space");
...
export default { events, filters, attr, whitespace, stringify };